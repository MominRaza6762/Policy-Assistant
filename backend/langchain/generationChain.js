...
/**
 * Generate compliance-aware, policy-checked HR drafts.
 * Supports leave, resignation, or custom requests based on document context.
 */
export async function generateDraft({
  ...
  // 🔹 Step 1: Retrieve relevant policy content from document
  ...
  // 🔹 Step 2: Compliance Check Before Drafting
...
`;
  ...
  // 🔹 Step 2.5: Smarter compliance handling
  let isAllowed = true;

  if (complianceText.startsWith("NOT ALLOWED")) {
    // If rejection reason is only due to missing context → still allow
  ...
  // 🔹 Step 3: Generate the actual draft if policy allows
  const systemPrompt = `
...
`;
  ...
  // 🔹 Step 4: Save the generated draft to DB
  const draft = await GeneratedDraft.create({
    userId,
    type,
    content,
